...
export default function BusEditar() {
  ...
  const cargarBus = async () => {
    try {
      setLoadingData(true);
      const bus = await busesService.getOne(parseInt(id!));
      setFormData({
        numero: bus.numero,
        ruta: bus.ruta,
        placa: bus.placa,
        modelo: bus.modelo || "",
        color: bus.color || "",
        capacidad: bus.capacidad ? bus.capacidad.toString() : "",
      });
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await busesService.update(parseInt(id!), {
      ...
      console.error("Error actualizando bus:", err);
      const errorMessage =
        err.response?.data?.message || "Error al actualizar bus";
  ...
  return (
    <div className="max-w-2xl">
      ...
      <div className="bg-white rounded-lg shadow-md p-8">
        <h1 className="text-3xl font-bold text-gray-800 mb-6">Editar Bus</h1>
